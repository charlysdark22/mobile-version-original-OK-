...
export default function CantinaScreen({ route }: any) {
  ...
  const loadData = useCallback(async () => {
    ...
    // Get canteen products from local inventory
    ...
    const bajoStock = productosCantina.filter(p => p.cantidad <= 5).length;
    
    setMetrics({ entradas, consumos, stockActual, bajoStock });
    
    // Get central stock for each product
    const inventarioCompleto = productosCantina.map(p => {
      const centralProduct = datos.almacenCentral.find(cp => 
        cp.nombre === p.nombre && cp.categoria === 'cantina'
      );
      return {
        ...p,
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.headerSection}>
        ...
        <View style={styles.headerText}>
          <Text style={[styles.title, { color: COLORS.text }]}>Cantina - Cafe Avellaneda</Text>
          <Text style={[styles.subtitle, { color: COLORS.muted }]}>Reportes e inventario de cantina</Text>
        </View>
      </View>
      ...
      <Card style={styles.inventoryCard}>
        <View style={styles.inventoryHeader}>
          <Text style={styles.inventoryTitle}>Inventario de Cantina - Local</Text>
        </View>
